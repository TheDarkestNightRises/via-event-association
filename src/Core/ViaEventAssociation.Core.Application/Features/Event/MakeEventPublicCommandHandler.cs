...
public class MakeEventPublicCommandHandler(IEventRepository repository, IUnitOfWork uow) : ICommandHandler<MakeEventPublicCommand>
{
    public async Task<Result<Void>> HandleAsync(MakeEventPublicCommand command)
    {
        var evt = await repository.GetAsync(command.Id);
        var result = evt.MakeEventPublic();
