...
public class MakeEventPrivateCommandHandler(IEventRepository repository, IUnitOfWork uow) : ICommandHandler<MakeEventPrivateCommand>
{
    public async Task<Result<Void>> HandleAsync(MakeEventPrivateCommand command)
    {
        var evt = await repository.GetAsync(command.Id);
        var result = evt.MakeEventPrivate();
